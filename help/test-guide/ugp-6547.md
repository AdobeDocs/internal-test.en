...
- [Dataflows](toc-overview.md): Dataflows are a representation of data jobs that move data across Platform. Dataflows are configured across different services, helping move data from source connectors to target datasets, to [!DNL Identity] and [!DNL Profile], and to [!DNL Destinations].
  - [Dataflow runs](toc-overview.md): Dataflow runs are the recurring scheduled jobs based on the frequency configuration of selected dataflows.
- [Destinations](toc-overview.md): Destinations are pre-built integrations with commonly used applications that allow for the seamless activation of data from Platform for cross-channel marketing campaigns, email campaigns, targeted advertising, and many other use cases.
- [Sandboxes](toc-overview.md): [!DNL Experience Platform] provides virtual sandboxes which partition a single [!DNL Platform] instance into separate virtual environments to help develop and evolve digital experience applications.
...
![Select destination view](assets/adobe-logo-old.png)
...
![Dataflow runs details for streaming destinations](assets/adobe-logo-old.png)
...
![Activation rate formula](assets/adobe-logo-old.png)
...
![Dataflow details for streaming destinations](assets/adobe-logo-old.png)
...
![Dataflow records for streaming destinations](assets/adobe-logo-old.png)
...
![Dataflow runs view for batch destinations](assets/adobe-logo-old.png)
...
>[!NOTE]
>
>Dataflow runs are generated based on the destination dataflow's schedule frequency. A separate dataflow run is made for each [merge policy](toc-overview.md) applied to a segment.
...
![Dataflow run details for batch destinations](assets/adobe-logo-old.png)
...
![Dataflow records for batch destinations](assets/adobe-logo-old.png)
...
>[!NOTE]
>
>- Destinations monitoring functionality is currently supported for all destinations in Experience Platform *except* the [Adobe Target](toc-overview.md) and [Custom personalization](toc-overview.md) destinations.
>- For the [Amazon Kinesis](toc-overview.md), [Azure Event Hubs](toc-overview.md), and [HTTP API](toc-overview.md) destinations, identities excluded are currently not displayed.

To access the [!UICONTROL Monitoring] dashboard, select **[!UICONTROL Monitoring]** (assets/adobe-logo-old.png)) in the left navigation. Once on the [!UICONTROL Monitoring] page, select [!UICONTROL Destinations]. The [!UICONTROL Monitoring] dashboard contains metrics and information on the destination run jobs.
...
![Streaming and batch activation graphs](assets/adobe-logo-old.png)
...
![Change lookback date range for activated destinations](assets/adobe-logo-old.png)

Use the arrow icon (assets/adobe-logo-old.png)) to expand or dismiss the cards at the top of the screen, which show at-a-glance information about the activation details, based on the destination type - streaming or batch:
...
![Dashboard all activated destinations](assets/adobe-logo-old.png)

You can also filter your list of destinations to only display the selected category of destinations. Select the **[!UICONTROL My destinations]** dropdown, and select the you want to filter to.

![Filter destinations using dropdown selector](assets/adobe-logo-old.png)

Additionally, you can enter a destination into the search bar to isolate to a single destination. If you want to see the destination's dataflows you can select the filter ![filter](assets/adobe-logo-old.png) beside it to see a list of its active dataflows.

![Filter destinations using search bar](assets/adobe-logo-old.png)

If you want to view all existing dataflows across all destinations, select **[!UICONTROL Dataflows]**.

A list of dataflows appears, sorted by the last dataflow run. You can see additional details for a specific dataflow by locating the destination you want to monitor, selecting the filter ![filter](assets/adobe-logo-old.png) beside it, and then subsequently selecting the filter ![filter](assets/adobe-logo-old.png) beside the dataflow you want more information about.

![All dataflows highlighted in the monitoring dashboard](assets/adobe-logo-old.png)
...
![Flow runs panel](assets/adobe-logo-old.png)
...
![Segments in dataflow panel](assets/adobe-logo-old.png)
...
Use the filter button ![filter](assets/adobe-logo-old.png) to drill down into the dataflow runs view for each segment in the dataflow.
...
![Filtering dataflow runs for segments](assets/adobe-logo-old.png)

To see more details about a specific dataflow run, select the filter ![filter](assets/adobe-logo-old.png) beside the dataflow run start time to see the dataflow run details page.

![Dataflow runs filter in monitoring dashboard](assets/adobe-logo-old.png)
...
![Identities excluded toggle](assets/adobe-logo-old.png)
...
![Dataflow run - segments view](assets/adobe-logo-old.png)

## Next steps {#next-steps}

By following this guide, you now know how to monitor dataflows for both batch and streaming destinations, including all the relevant information such as processing time, activation rate, and status. To learn more about dataflows in Platform, please read the [dataflows overview](toc-overview.md). To learn more about destinations, please read the [destinations overview](toc-overview.md).